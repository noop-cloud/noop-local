...
class ComponentContainer extends Container {
  ...
  getEnv () {
    ...
    if (this.component.type === 'static') {
      env.NOOP_STATIC_INDEX_FILE = this.component.settings.indexFile
      env.NOOP_STATIC_SINGLE_PAGE_MODE = this.component.settings.singlePageMode
    }
  ...
  getEnvInheritance () {
    ...
    // statics
    if (this.component.type === 'static') {
      variables.NOOP_STATIC_INDEX_FILE = [{ source: 'static', value: this.component.settings.indexFile }]
      variables.NOOP_STATIC_SINGLE_PAGE_MODE = [{ source: 'static', value: this.component.settings.singlePageMode }]
    }
    // noopfile
    Object.keys(this.component.variables).forEach((key) => {
  ...
  build (done) {
    ...
    async.auto({
      ...
      buildImage: ['addDockerfile', (results, done) => {
        tarfs.pack(this.component.rootPath, {
          ignore: (name) => this.buildIgnore(name),
          pack: pack
        })
          ...
          async.auto({
            build: async (done) => {
              async.doWhilst(
                ...
                (...args) => {
                  args.pop()(null, this.reloadNeeded)
                },
                done
              )
            },
  ...
  buildIgnore (file) {
    const ignoreStatus = this.devServer.watcher.checkIgnoreStatus(file)
    if (typeof ignoreStatus === 'boolean') return ignoreStatus
    const stat = fs.statSync(file)
    if (stat.isDirectory()) {
      return false
    } else {
      const watchStatus = this.devServer.watcher.checkWatchStatus(file, this.component.name)
